...
function get_time_depending_style(&$vars)
{
    ...
    $vars['day_night_sky_santiago']= "background-image: url('".base_path().file_directory_path()."/images/santiagoDe".$dia_o_noche.".png');";     
...
function basic_preprocess_page(&$vars, $hook) {
  ...
  if(!$vars['is_front'])
  {
      $vars['styles'].='<link href="' .base_path().drupal_get_path('theme', $GLOBALS['theme']).'/css/style-front.css" rel= "stylesheet" type="text/css">';
        
      
  }
  
  else
      get_time_depending_style($vars);
...
function buildSecundaryLinks($secundaryLinks)
{
    
    $tree = menu_tree_all_data('secondary-links');

    $markup = "";
    
    foreach($tree as $menuItemLvl1Key => $menuItemLvl1Value)
    {
       
       $markup.= '<ul id="'.$menuItemLvl1Value['link']['title'].'" class="secondary-links">';
       $markup.= '<li class="'.$menuItemLvl1Value['link']['options']['attributes']['class'].'">'.$menuItemLvl1Value['link']['title'];
       if($menuItemLvl1Value['below'])
       {
           $markup.= '<ul>';
           foreach($menuItemLvl1Value['below'] as $menuItemLvl2Key => $menuItemLvl2Value)
           {
               
               $markup.= '<li>'.'<a class="'.$menuItemLvl2Value['link']['options']['attributes']['class'].'" href="'.$menuItemLvl2Value['link']['href'].'">'.$menuItemLvl2Value['link']['title'].'</a></li>';
           }
           $markup.='</ul>';
       }
       $markup.='</li>';
 ...
 * 
 */
function basic_preprocess_ddblock_cycle_block_content(&$vars) {
 ...
 *
 */
function basic_preprocess_ddblock_cycle_pager_content(&$vars) {
